...
async function checkGithub(): Promise<HealthItem> {
  ...
  if (!token) {
    return { id: "github", label: "GitHub Release API", level: "warning", message: "GITHUB_RELEASE_TOKEN fehlt.", latencyMs: null, critical: false };
  }
...
async function checkVercel(): Promise<{ health: HealthItem; deployments: VercelDeployment[] }> {
  ...
  if (!token) {
    return { health: { id: "vercel", label: "Vercel Deployment API", level: "warning", message: "VERCEL_API_TOKEN fehlt.", latencyMs: null, critical: false }, deployments: [] };
  }

  const started = Date.now();
  const teamQuery = teamId ? `?teamId=${encodeURIComponent(teamId)}` : "";
  try {
    const projectResponse = await fetch(`https://api.vercel.com/v9/projects/${encodeURIComponent(projectRef)}${teamQuery}`, {
    ...
    const deploymentResponse = await fetch(`https://api.vercel.com/v6/deployments?${params.toString()}`, {
      headers: { Authorization: `Bearer ${token}` }, cache: "no-store", signal: AbortSignal.timeout(5000),
    });
    ...
    const deployments: VercelDeployment[] = Array.isArray(deploymentBody.deployments)
    ...
    return {
      health: {
        ...
        message: failed > 0 ? `${failed} fehlgeschlagene Deployments in den letzten ${deployments.length}` : running > 0 ? `${running} Deployment(s) laufen gerade` : `${projectBody.name ?? projectRef} erreichbar`,
...
async function persistAndLoadStability(adminClient: any, items: HealthItem[]): Promise<{ available: boolean; items: StabilityItem[] }> {
  const now = new Date();
  const fiveMinutesAgo = new Date(now.getTime() - 5 * 60 * 1000).toISOString();
  const dayAgo = new Date(now.getTime() - 24 * 60 * 60 * 1000).toISOString();
  const eightDaysAgo = new Date(now.getTime() - 8 * 24 * 60 * 60 * 1000).toISOString();

  const recentCheck = await adminClient.from("system_health_samples").select("id").gte("checked_at", fiveMinutesAgo).limit(1);
  if (recentCheck.error) return { available: false, items: [] };

  if ((recentCheck.data ?? []).length === 0) {
    await adminClient.from("system_health_samples").insert(items.map((item) => ({
      service_id: item.id,
      level: item.level,
    ...
    void adminClient.from("system_health_samples").delete().lt("checked_at", eightDaysAgo);
  }

  const history = await adminClient
    .from("system_health_samples")
    .select("service_id,level,latency_ms,checked_at")
    .gte("checked_at", dayAgo)
    .order("checked_at", { ascending: true })
    .limit(3000);
  if (history.error) return { available: false, items: [] };

  const rows = history.data ?? [];
  const result: StabilityItem[] = items.map((service) => {
    const serviceRows = rows.filter((row: any) => row.service_id === service.id);
    const okCount = serviceRows.filter((row: any) => row.level === "ok").length;
    const latencies = serviceRows.map((row: any) => row.latency_ms).filter((value: unknown): value is number => typeof value === "number");
    const buckets: HealthLevel[][] = Array.from({ length: 24 }, () => []);
    for (const row of serviceRows) {
      const ageMs = now.getTime() - new Date(row.checked_at).getTime();
      const bucket = 23 - Math.min(23, Math.max(0, Math.floor(ageMs / (60 * 60 * 1000))));
      buckets[bucket].push(row.level as HealthLevel);
    }
    return {
      id: service.id,
      label: service.label,
      uptime24h: serviceRows.length ? Math.round((okCount / serviceRows.length) * 10000) / 100 : null,
      avgLatencyMs: latencies.length ? Math.round(latencies.reduce((sum, value) => sum + value, 0) / latencies.length) : null,
      problems24h: serviceRows.filter((row: any) => row.level !== "ok").length,
      samples24h: serviceRows.length,
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const stability = await persistAndLoadStability(context.adminClient, items);
